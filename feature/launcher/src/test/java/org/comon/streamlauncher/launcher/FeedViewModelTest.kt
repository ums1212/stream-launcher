...
class FeedViewModelTest {
    ...
    fun setUp() {
        ...
        every { getIntegratedFeedUseCase(any()) } returns flowOf(Result.success(sampleFeedItems))
    ...
    fun `초기 상태 - 기본값 확인`() {
        ...
        every { getIntegratedFeedUseCase(any()) } returns flowOf(Result.success(emptyList()))
    ...
    fun `Refresh - 피드 새로 로드`() = runTest {
        ...
        val newItems = listOf(FeedItem.VideoItem("새영상", 999L, "thumb", "link"))
        every { getIntegratedFeedUseCase(any()) } returns flowOf(Result.success(newItems))
